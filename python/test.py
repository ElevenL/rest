...
logging.basicConfig(level=logging.DEBUG,
                ...
                filename='/root/okex/rest/python/test.log',
                # filename='log',
                filemode='a')
...
def worker(interval):
    while(1):
        print("work start:{0}".format(time()))
        sleep(interval)
        print("work end:{0}".format(time()))
...
if __name__ == '__main__':
    ...
    for s in SYMBOL[:10]:
        ...
        ps.append(multiprocessing.Process(target=api.getDepth, args=(p2,)))
        ps.append(multiprocessing.Process(target=api.policy, args=(symbols,)))
    for p in ps:
        p.daemon = True
    for p in ps:
        p.start()
